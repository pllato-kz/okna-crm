  ...
  if(tab==='profile'){
    ...
    const rows=list.map(m=>{const low=m.stock<m.min; const pct=Math.min(100,m.stock/(m.min*2)*100); const bb=barBreakdown(m); const cost=matCost(m); const barLen=m.barLen||6;
      return `<tr data-wh-row="${m.id}"><td><div style="font-weight:600">${escA(m.name)}</div><div class="muted2" style="font-size:11.5px">${escA(m.supplier)}</div></td>
        ...
        <td style="min-width:120px"><div style="display:flex;align-items:center;gap:8px"><div class="mini-bar"><i style="width:${pct}%;background:${low?'var(--red)':'var(--green)'}"></i></div>
          <span><b style="white-space:nowrap">${bb.bars} хлыст.</b> <span class="muted2">(по ${barLen} м)</span> · <span style="white-space:nowrap">${m.stock} пог.м</span>${bb.offcutTotal>0?` <span class="tag green" data-act="wh-offcuts" data-id="${m.id}" style="font-size:10px;cursor:pointer;white-space:nowrap" title="Показать список обрезков">обрезки: ${bb.offcuts.length} шт · ${bb.offcutTotal} м</span>`:''}</span></div></td>
