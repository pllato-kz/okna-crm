...
function apiMapBootstrap(boot){
  ...
  const deals = (boot.deals || []).map(d => ({
    ...
    items: (d.items || []).map(it => ({
      ...
      priceOverride: it.price_override != null ? it.price_override : null,
    })),
...
const apiPersist = {
  ...
  createItem: (dealId, it) => apiFetch('deal_items', { method: 'POST', body: {
    ...
    w: it.w, h: it.h, sashes: it.sashes, sashes_json: it.sashList ? JSON.stringify(it.sashList) : null, price_override: it.priceOverride != null ? it.priceOverride : null, qty: it.qty,
  }}),
  saveItem:   (it) => apiFetch('deal_items/' + it.id, { method: 'PUT', body: {
    profile_id: it.profileId, glass_id: it.glassId, opening_id: it.openId, w: it.w, h: it.h, sashes: it.sashes, sashes_json: it.sashList ? JSON.stringify(it.sashList) : null, price_override: it.priceOverride != null ? it.priceOverride : null, qty: it.qty,
  }}),
