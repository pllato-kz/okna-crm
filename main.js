...
function stageDelModal(id){
  if(!isDirector()) return; const s=stageById(id); if(!s) return;
  if(SYSTEM_STAGE_IDS.includes(id)){ toast('Системную стадию (замер/производство/монтаж/выполнено/проиграно) удалить нельзя. Можно переименовать и сменить цвет','warn'); return; }
