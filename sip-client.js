...
(function () {
  ...
  async function fetchCreds() {
    // ОКНА CRM: JWT лежит в localStorage под ключом okna_crm_token (см. api.js),
    // API — тот же origin с префиксом /api.
    const token = localStorage.getItem('okna_crm_token');
    const apiBase = (window.PLLATO_CONFIG && window.PLLATO_CONFIG.API_URL) || '';
    const resp = await fetch(apiBase + '/api/sip/token', {
  ...
  async function logCallEnded(meta) {
    try {
      ...
      await fetch(apiBase + '/api/sip/log', {
        method: 'POST',
        headers: {
